...
# structures using GNINA CNN scoring. Computes consensus box for
# the 4-structure rigid ensemble.
#
# Usage:
#   bash submit_box_optimization.sh          # GPU (recommended)
#   bash submit_box_optimization.sh cpu      # CPU fallback
#=====================================================================

PROJECT_DIR=~/final_project
OUTPUT_DIR="${PROJECT_DIR}/box_optimization"
POCKET_DIR="${PROJECT_DIR}/pocket_analysis"
SCRIPT="${PROJECT_DIR}/box_optimization.py"
...
SLURM_SCRIPT="${OUTPUT_DIR}/slurm_box_optimization.sh"

cat > "${SLURM_SCRIPT}" << 'SLURM_EOF'
#!/bin/bash
#SBATCH --job-name=boxopt
#SBATCH --partition=PARTITION_VAL
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=NTASKS_VAL
#SBATCH --mem=MEM_VAL
#SBATCH --time=TIME_VAL
#SBATCH --output=OUTPUT_DIR_VAL/logs/boxopt_%j.out
#SBATCH --error=OUTPUT_DIR_VAL/logs/boxopt_%j.err
ACCOUNT_VAL
GPU_LINE_VAL
...
echo "Check output: tail -f ${RESULTS_DIR}/boxopt_*.out"
echo "Results: ${OUTPUT_DIR}/box_optimization_report.txt"
echo "         ${OUTPUT_DIR}/box_optimization_results.json"
echo "Plots:   ${OUTPUT_DIR}/box_optimization_sweep.png"
echo "         ${OUTPUT_DIR}/box_consensus_2d.png"
echo "Viz:     ${OUTPUT_DIR}/visualize_box.py"
