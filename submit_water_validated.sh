...
#
# Re-runs cross-docking IFP matrix with conserved waters embedded
# in receptor. Compares dry vs wet docking to validate water placement.
#
# Usage:
#   bash submit_water_validated.sh          # GPU (recommended)
#   bash submit_water_validated.sh cpu      # CPU fallback
#=====================================================================

PROJECT_DIR=~/final_project
OUTPUT_DIR="${PROJECT_DIR}/water_validated_docking"
POCKET_DIR="${PROJECT_DIR}/pocket_analysis"
SCRIPT="${PROJECT_DIR}/water_validated_docking.py"
...
echo "============================================"
echo "Water-Validated Cross-Docking - Task 6"
echo "============================================"
...
SLURM_SCRIPT="${OUTPUT_DIR}/slurm_water_validated.sh"

cat > "${SLURM_SCRIPT}" << 'SLURM_EOF'
#!/bin/bash
#SBATCH --job-name=watdock
#SBATCH --partition=PARTITION_VAL
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=NTASKS_VAL
#SBATCH --mem=MEM_VAL
#SBATCH --time=TIME_VAL
#SBATCH --output=OUTPUT_DIR_VAL/logs/watdock_%j.out
#SBATCH --error=OUTPUT_DIR_VAL/logs/watdock_%j.err
ACCOUNT_VAL
GPU_LINE_VAL
...
echo "Check output: tail -f ${RESULTS_DIR}/watdock_*.out"
echo "Results: ${OUTPUT_DIR}/water_validated_report.txt"
echo "         ${OUTPUT_DIR}/water_validated_results.json"
echo "Plot:    ${OUTPUT_DIR}/water_validated_comparison.png"
